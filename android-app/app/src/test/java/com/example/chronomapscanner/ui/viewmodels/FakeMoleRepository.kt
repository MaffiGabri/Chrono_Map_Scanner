...
class FakeMoleRepository : MoleRepository {
    ...
    override fun getMolesForMap(profileName: String, side: com.example.chronomapscanner.data.domain.BodySide, maxDate: String, colors: List<String>): Flow<List<com.example.chronomapscanner.data.domain.MoleMapItem>> {
        return moles.map { allMoles ->
    ...
    override suspend fun updateMolePosition(id: String, x: Float, y: Float, side: com.example.chronomapscanner.data.domain.BodySide) {
        val current = moles.value.toMutableList()
