...
import com.example.chronomapscanner.ui.BodyMapUiState
...
class BodyMapViewModel @Inject constructor(
    ...
    private val uiMolesAndCountsFlow = combine(
        ...
        val filteredMoles = availableMolesForSide.filter { mole -> 
            ...
            com.example.chronomapscanner.ui.MoleUiModel(
                id = baseDto.id,
                x = baseDto.x,
                y = baseDto.y,
    ...
    private var cachedGrid: Map<Int, List<Pair<Int, com.example.chronomapscanner.ui.MoleUiModel>>>? = null
    private var lastMolesRef: List<com.example.chronomapscanner.ui.MoleUiModel>? = null
    ...
    suspend fun findMoleAtTap(internalX: Float, internalY: Float, canvasWidth: Float, canvasHeight: Float, thresholdSq: Float): String? = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Default) {
        ...
        val grid = gridMutex.withLock {
            if (cachedGrid == null || lastMolesRef !== uiMoles) {
                val newGrid = mutableMapOf<Int, MutableList<Pair<Int, com.example.chronomapscanner.ui.MoleUiModel>>>()
